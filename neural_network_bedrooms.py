...
class AirbnbNightlyPriceImageDataset(Dataset):
    ...
    def __init__(self):
        ...
        clean_data = pd.concat([clean_data, one_hot], axis=1)
        print(clean_data)
        self.features, self.label = load_airbnb(clean_data, 'bedrooms')
...
class NeuralNetwork(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_input_layer = torch.nn.Linear(16,5)
...
def find_best_nn(yaml_file, model, dataset, folder):
    ...
    metrics_files = glob.glob("./neural_networks/regression_bedrooms/*/metrics.json", recursive=True)
    best_score = 0
    ...
    path = f'./neural_networks/regression_bedrooms/{best_name}/'
...
if __name__ == '__main__':
    ...
    model = NeuralNetwork()
    best_name, params, metrics = find_best_nn('nn_config.yaml', model, dataset,'neural_networks/regression_bedrooms')
